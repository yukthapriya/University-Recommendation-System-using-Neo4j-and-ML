...
uni = pd.read_csv('/Users/yukthapriya/Documents/University/university_data.csv')
...
def get_recommendations(connection, college_name):
    ...
    YIELD node1, node2, similarity
    WITH gds.util.asNode(node1) AS university1, gds.util.asNode(node2) AS university2, similarity
    WHERE university1.name = $college_name AND university1 <> university2
      AND similarity > 0  // Ensure only meaningful similarity scores are considered
    RETURN university1.name AS University, university2.name AS SimilarUniversity, similarity
    ORDER BY similarity DESCENDING, University, SimilarUniversity
    """
    parameters = {'college_name': college_name}
    result = connection.query(query, parameters)
    ...
    if result:
        print(f"Recommendations for {college_name}: {result}")
    else:
        print(f"No recommendations found for {college_name}")
